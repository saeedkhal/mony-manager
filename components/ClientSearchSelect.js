...
import {
  CLIENT_SELECT_DEFAULT_LIMIT,
  CLIENT_SELECT_MIN_CHARS,
  searchClientsForSelect,
} from "../utils/db";
...
export default function ClientSearchSelect({
  ...
  useEffect(() => {
    ...
    const searching = trimmed.length >= CLIENT_SELECT_MIN_CHARS;
    const q = searching ? trimmed : "";
    const delay = searching ? SEARCH_DEBOUNCE_MS : 0;
    let cancelled = false;
    const timer = setTimeout(() => {
      setLoading(true);
      searchClientsForSelect(q, searching ? SEARCH_RESULT_LIMIT : CLIENT_SELECT_DEFAULT_LIMIT)
        .then((rows) => {
  ...
  const showMinHint = typedLen > 0 && typedLen < CLIENT_SELECT_MIN_CHARS;
  const displayLabel = selectedLabel || "اختر العميل";
  ...
  let emptyText = "";
  if (showMinHint) emptyText = `اكتب ${CLIENT_SELECT_MIN_CHARS} حروف على الأقل`;
  else if (results.length === 0 && !loading) {
    emptyText =
      typedLen >= CLIENT_SELECT_MIN_CHARS
        ? "لا توجد نتائج"
        : "لا يوجد عملاء";
  }
              ...
              <View style={local.combo}>
                ...
                <View style={local.dropdown}>
                  {loading && results.length === 0 ? (
                    ...
                    >
                      {results.map((c) => {
                        const selected = Number(value) === Number(c.id);
                        return (
                          <TouchableOpacity
                            key={c.id}
